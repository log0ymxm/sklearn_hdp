...
from ..hdp import HierarchicalDirichletProcess as HDP
import numpy as np
import unittest
...
class TestHDP(unittest.TestCase):
    ...
    def sequence_random(self, alpha, beta, gamma, seed):
        ...
        model = HDP(alpha=alpha, beta=beta, gamma=gamma, verbose=True)
        model._setup(docs)
    ...
    def sequence4(self, alpha, beta, gamma):
        ...
        k1 = model._add_new_dish()
        k2 = model._add_new_dish()

        j = 0
        t1 = model._add_new_table(j, k1)
        t2 = model._add_new_table(j, k2)
        model._seat_at_table(j, 0, t1)
        model._seat_at_table(j, 1, t2)
        model._seat_at_table(j, 2, t2)
        model._seat_at_table(j, 3, t2)

        j = 1
        t1 = model._add_new_table(j, k1)
        t2 = model._add_new_table(j, k2)
        model._seat_at_table(j, 0, t2)
        model._seat_at_table(j, 1, t2)
        model._seat_at_table(j, 2, t1)
        model._seat_at_table(j, 3, t2)
        ...
        model._leave_from_dish(2, 1)
        model._seat_at_dish(2, 1, 2)


        model._leave_from_table(2, 0)
        model._seat_at_table(2, 0, 2)


        model._leave_from_dish(0, 1)
        model._seat_at_dish(0, 1, 2)
        self.assertEqual(model.m, 5)
        self.assertEqual(model.m_k[1], 1)
        self.assertEqual(model.m_k[2], 4)

        model._leave_from_dish(1, 1)
        model._seat_at_dish(1, 1, 2)

        model._leave_from_table(2, 3)
        k_new = model._add_new_dish()
        self.assertEqual(k_new, 1)
        t_new = model._add_new_table(j, k_new)
        self.assertEqual(t_new, 1)
        model._seat_at_table(2, 3, 1)
        ...
        j = 0
        t = 1
        model._leave_from_dish(j, t)

        #print("n_jt=", model.n_jt[j][t])

        p_k = model._calc_dish_posterior_t(j, t)
        #print("p_k=", p_k)
        p0 = gamma / V
        p1 = 1 * beta / (V * beta + 1)
    ...
    def sequence2(self, alpha, beta, gamma):
        ...
        for j in xrange(3):
            ...
            self.assertEqual(t_new, 1)
            for i in xrange(4):
                model._seat_at_table(j, i, t_new)
        ...
        model._leave_from_dish(0, 1) # decrease m and m_k only
        self.assertEqual(model.m, 2)
        self.assertEqual(model.m_k[1], 2)

        model._seat_at_dish(0, 1, 1)
        self.assertEqual(model.m, 3)
        self.assertEqual(model.m_k[1], 3)

        for i in xrange(1):
            for j in xrange(3):
                model._sampling_k(j, 1)
                #model.dump()
    ...
    def sequence1(self, alpha, beta, gamma):
        ...
        p_k = model._calc_dish_posterior_w(f_k)
        ...
        self.assertEqual(model.n_jt[j][t_new], 0) # still 0

        model._seat_at_table(j, i, t_new)
        self.assertEqual(model.t_ji[j][i], 1)
        self.assertEqual(model.n_jt[j][t_new], 1) # increase was
        ...
        self.assertEqual(model.n_jt[j][t_new], 2) # increase was
        self.assertEqual(model.n_kv[k_new][v], beta + 1)
